...
class PatientDataset(Dataset):
                        ...
                        for day_index in day_indices:
                            ...
                            if mode == "train":
                                ...
                                for start_idx in range(0, len(day_data) - self.window_size, self.stride):
                                    ...
                                    # only last timestep of sequence
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            ...
            if len(day_data) == self.window_size:
                ...
                for start_idx in range(0, len(day_data) - self.window_size, self.window_size // 3):
                    ...
                    # last timestep as target
                    target = day_data.iloc[start_idx + self.window_size][self.target_columns].copy().to_numpy()
        ...
        return {
            ...
            "target": target_tensor,  # circadian (sin, cos)
            "user_id": torch.tensor(patient_id, dtype=torch.long) - 1,
            "relapse_label": torch.tensor(relapse_label, dtype=torch.long),  # 🔑 always available
            "idx": idx,
        }
