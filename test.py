...
def main():
    ...
    # Total AUROC/AUPRC (for both val and test)
    if len(all_auroc) > 0:
        total_auroc = np.nanmean(all_auroc)
        total_auprc = np.nanmean(all_auprc)
        total_avg = np.nanmean([total_auroc, total_auprc])

        print(f'\n==== FINAL RESULTS ({args.mode.upper()} MODE) ====')
        print(f'Total AUROC: {total_auroc:.4f}')
        print(f'Total AUPRC: {total_auprc:.4f}')
        print(f'Total AVG: {total_avg:.4f}')
        print(f'Random AUROC: 0.5000')
        print(f'Random AUPRC: {np.nanmean(random_auprc):.4f}')
        print(f'Ideal AVG: {(0.5 + np.nanmean(random_auprc)) / 2:.4f}')
