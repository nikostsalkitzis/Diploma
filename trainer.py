...
class Trainer:
    """ Class to train the multi-task Transformer (circadian + relapse) """
    ...
    def __init__(self, models, optims, scheds, loaders, args):
        ...
        # weight for relapse classification loss
        ...
        # Ensemble MLP heads for anomaly detection (optional)
        self.mlps = []
        self.optimizers = []
    ...
    def train_encoder_once(self, epoch: int, i: int, epoch_metrics: dict):
        """ Train Transformer encoder on circadian + relapse tasks """
        self.models[i].train()
        for batch in tqdm(self.dataloaders[i]["train"], desc=f"Train Encoder ({i+1}/{len(self.models)})"):
            ...
            circadian_labels = batch["target"].to(self.args.device)   # (batch, 2, 1)
            circadian_labels = torch.squeeze(circadian_labels, dim=-1)  # (batch, 2)

            relapse_labels = batch["relapse_label"].float().to(self.args.device)  # (batch,)

            # Forward pass
            features, circadian_preds, relapse_logits = self.models[i](x)

            # Losses
            loss_circadian = self.regression_loss(circadian_preds, circadian_labels)
            loss_relapse = self.classification_loss(relapse_logits.squeeze(), relapse_labels)

            total_loss = loss_circadian + self.alpha * loss_relapse
            ...
            metrics = {
                ...
                "loss_circadian": loss_circadian.item(),
                "loss_relapse": loss_relapse.item(),
            }
    ...
    # (ensemble-related methods remain the same, for anomaly detection)
    ...
    def train_ensemble(self, i: int):
        """ Train ensemble MLP on top of encoder features """
    ...
    def evaluate_ensembles(self, epoch: int, i: int, train_dist_anomaly_scores: dict):
        """ Validation: anomaly detection (same as before) """
        k = self.args.ensembles
        anomaly_scores = []
        relapse_labels = []
        user_ids = []
